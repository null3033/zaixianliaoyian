...
export function setupMobileUIHandlers() {
  ...
  const rightbar = window.DOMNodes ? window.DOMNodes.rightbar : null;
...
export function renderUserList(updateHeader = false) {
  // 使用固定节点名称
  const userListEl = window.DOMNodes ? window.DOMNodes.memberList : null;
...
export function initLoginForm() {
  // 使用固定节点名称
  const loginFormContainer = window.DOMNodes ? window.DOMNodes.loginForm : null;
